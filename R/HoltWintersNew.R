...
)
{
	...
	if((is.null(alpha))&&(is.null(beta))&&(is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = p[1L], beta=p[2L], gamma = p[3L], start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
	...
	if((is.null(alpha))&&(!is.null(beta))&&(is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = p[1L], beta=beta, gamma = p[2L], start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
		sol   <- optim(optim.start, error, method = "L-BFGS-B",
	...
	#beta, gamma are null
	if((!is.null(alpha))&&(is.null(beta))&&(is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = alpha, beta=p[1L], gamma = p[2L], start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
		sol   <- optim(optim.start, error, method = "L-BFGS-B",
				lower = c(0, 0), upper = c(1, 1),
				control = optim.control)
		if(sol$convergence || any(sol$par < 0 | sol$par > 1)) {
	...
	#alpha, beta are null
	if((is.null(alpha))&&(is.null(beta))&&(!is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = p[1L], beta=p[2L], gamma = gamma, start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
		sol   <- optim(optim.start, error, method = "L-BFGS-B",
				lower = c(0, 0), upper = c(1, 1),
				control = optim.control)
		if(sol$convergence || any(sol$par < 0 | sol$par > 1)) {
	...
	#alpha is null
	if((is.null(alpha))&&(!is.null(beta))&&(!is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = p[1L], beta=beta, gamma = gamma, start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
		sol   <- optim(optim.start, error, method = "L-BFGS-B",
				lower = c(0), upper = c(1),
				control = optim.control)
		if(sol$convergence || any(sol$par < 0 | sol$par > 1)) {
	...
	if((!is.null(alpha))&&(is.null(beta))&&(!is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = alpha, beta=p[1L], gamma = gamma, start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
		sol   <- optim(optim.start, error, method = "L-BFGS-B",
				lower = c(0), upper = c(1),
				control = optim.control)
		if(sol$convergence || any(sol$par < 0 | sol$par > 1)) {
	...
	#gamma is null
	if((!is.null(alpha))&&(!is.null(beta))&&(is.null(gamma))){
		error <- function (p) zzhw(x,lenx=lenx, alpha = alpha, beta=beta, gamma = p[1L], start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)$SSE
		sol   <- optim(optim.start, error, method = "L-BFGS-B",
				lower = c(0), upper = c(1),
				control = optim.control)
		if(sol$convergence || any(sol$par < 0 | sol$par > 1)) {
	...
	final.fit <- zzhw(x,lenx=lenx, alpha = alpha, beta=beta, gamma = gamma, start.time=start.time, seasonal=seasonal, frequency=f, dotrend=(!is.logical(beta) || beta), doseasonal=(!is.logical(gamma) || gamma), l.start=l.start,b.start=b.start,s.start=s.start)
