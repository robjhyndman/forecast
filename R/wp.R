tswp <- function(formula,data,lambda=NULL,model="gam",...)
{
  ...
  if(model=="lm"){
    ...
    fit <- gam(formula,family=gaussian,data=data,na.action=na.exclude,...)
  }
...
forecast.wp <- function(object, newdata, h=10, level=c(80,95), fan=FALSE, lambda=object$lambda, ts=TRUE, ...)
{
  if (fan)
  ...
  } else if(is.element("gam", class(object))){
    fcast$method <- "Generalized additive model"
  }
