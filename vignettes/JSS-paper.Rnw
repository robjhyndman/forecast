...
\author{Rob J. Hyndman\\Monash University
   \And Yeasmin Khandakar\\Monash University}
\Plainauthor{Rob J. Hyndman, Yeasmin Khandakar}
\title{Automatic Time Series Forecasting: The \pkg{forecast} Package for \proglang{R}}
\Plaintitle{Automatic Time Series Forecasting: The forecast Package for R}

\Abstract{
This vignette to the \proglang{R} package \pkg{forecast} is an extended version of \cite{HK2008}, published in the \emph{Journal of Statistical Software}.

Automatic forecasts of large numbers of univariate time
series are often needed in business and other contexts. We describe
two automatic forecasting algorithms that have been implemented in
the \pkg{forecast} package for \R. The first is based on innovations
state space models that underly exponential smoothing methods. The
...
data, and are compared and illustrated using four real time series.
We also briefly describe some of the other functionality available
...
\Plainkeywords{ARIMA models, automatic forecasting, exponential smoothing, prediction intervals, state space models, time series, R}
...
\Address{
  ...
  E-mail: \email{Rob.Hyndman@buseco.monash.edu.au}\\
  URL: \url{http://www.robhyndman.info}\\

  Yeasmin Khandakar\\
  Department of Econometrics \& Business Statistics\\
  Monash University\\
  Clayton VIC 3800, Australia\\
  E-mail: \email{Yeasmin.Khandakar@buseco.monash.edu.au}
}
...
%\VignetteIndexEntry{Automatic Time Series Forecasting: The forecast Package for R}
%\VignetteDepends{forecast,expsmooth}
%\VignetteEngine{knitr::knitr}
%\VignetteKeywords{ARIMA models, automatic forecasting, exponential smoothing, prediction intervals, state space models, time series, R}
%\VignettePackage{forecast}


<<preliminaries, echo=FALSE>>=
library("forecast")
library("expsmooth")
oldopts <- options(prompt = "R> ", continue = "+   ")
@

\section{Introduction}

Automatic forecasts of large numbers of univariate time series are
often needed in business. It is common to have over one thousand
product lines that need forecasting at least monthly. Even when a
smaller number of forecasts are required, there may be nobody
suitably trained in the use of time series models to produce them.
In these circumstances, an automatic forecasting algorithm is an
essential tool. Automatic forecasting algorithms must determine an
appropriate time series model, estimate the parameters and compute
the forecasts. They must be robust to unusual time series patterns,
and applicable to large numbers of series without user intervention.
The most popular automatic forecasting algorithms are based on
either exponential smoothing or ARIMA models.

In this article, we discuss the implementation of two automatic
univariate forecasting methods in the \pkg{forecast} package for \R.
We also briefly describe some univariate forecasting
methods that are part of the \pkg{forecast} package.

The \pkg{forecast} package for the \proglang{R} system for
statistical computing \citep{R} is part of the \pkg{forecasting}
bundle \citep{expsmooth,fma,forecast,Mcomp} which also contains the packages \pkg{fma}, \pkg{expsmooth}
and \pkg{Mcomp}, and is available from the Comprehensive \proglang{R}
Archive Network at \url{http://CRAN.R-project.org/package=forecasting}. Version~1.11 of the
package was used for this paper. The \pkg{forecast} package contains
functions for univariate forecasting, while the other three packages
contain large collections of real time series data that are suitable
for testing forecasting methods. The \pkg{fma} package contains the
90~data sets from \citet{MWH3}, the \pkg{expsmooth} package contains
24~data sets from \citet{expsmooth08}, and the \pkg{Mcomp} package
contains the 1001~time series from the M-competition \citep{Mcomp82}
and the 3003~time series from the M3-competition \citep{M3comp00}.

The \pkg{forecast} package implements automatic forecasting using
exponential smoothing, ARIMA models, the Theta method \citep{AN00},
cubic splines \citep{HKPB05}, as well as other common forecasting
methods. In this article, we primarily discuss the exponential
smoothing approach (in Section~\ref{sec:expsmooth}) and the ARIMA
modelling approach (in Section~\ref{sec:arima}) to automatic
forecasting. In Section~\ref{sec:package}, we describe the implementation of these
methods in the \pkg{forecast} package, along with other features of the package.
...
\citet{OKS97}, \citet{HKSG02} and \citet{HKOS05} have shown that all
...
\citet{Gardner85}, modified by \citet{HKSG02}, and extended again by
\citet{Taylor03a}, giving a total of fifteen methods seen in the
following table.
...
\subsection{Point forecasts for all methods}
...
\begin{subequations}\label{AMmethod}
\begin{eqnarray}
...
\eqref{AMmethod}, we need values for the initial states $\ell_0$,
...
Holt-Winters equations such as those in \citet{MWH3} or
\citet{BOK05}. These authors replace \eqref{eq:3-46a} with
$$
...
modification given in \eqref{eq:3-46a} was proposed by \citet{OKS97}
...
is constant over time.  At the other extreme, na\"{\i}ve forecasts (i.e.,
...
\subsection{Innovations state space models} \label{sec:statespace}


For each exponential smoothing method in Table~\ref{table:pegels},
\citet{expsmooth08} describe two possible innovations state space
...
\end{eqnarray}
We simplify the last expression by setting $\beta=\alpha\beta^*$.
The three equations above constitute a state space model underlying
the damped Holt's method. Note that it is an \emph{innovations}
state space model \citep{AM79,Aoki87} because the same error term
appears in each equation. We can write it in standard state space
notation by defining the state vector as $\bm{x}_t = (\ell_t,b_t)'$
and expressing \eqref{ss1}--\eqref{ss3} as
\begin{subequations}
\begin{eqnarray}
...
\subsection{State space models for all exponential smoothing methods}\label{sec:ssmodels}
...
model is given in \citet{expsmooth08}.
...
estimates. \citet{OKS97} show that
...
$$
However, \citet{HAA08} show that these restrictions are usually
...
\subsection{Automatic forecasting}
\label{sec:algorithm}
...
\item Obtain prediction intervals for the best model either using
the analytical results of \citet{HKOS05}, or by simulating future
sample paths for $\{y_{n+1},\dots,y_{n+h}\}$ and finding the
$\alpha/2$ and $1-\alpha/2$ percentiles of the simulated data at
each forecasting horizon.  If simulation is used, the sample paths
may be generated using the normal distribution for errors
(parametric bootstrap) or using the resampled errors (ordinary
bootstrap).
\end{compactenum}

\citet{HKSG02} applied this automatic forecasting strategy to the
M-competition data \citep{Mcomp82} and the IJF-M3 competition data
\citep{M3comp00} using a restricted set of exponential smoothing
models, and demonstrated that the methodology is particularly good
at short term forecasts (up to about 6 periods ahead), and
especially for seasonal short-term series (beating all other methods
in the competitions for these series).
...
\citet{HR82} proposed a method to identify the order of an ARMA
...
\pkg{Forecast Pro} \citep{ForecastPro00} is well-known for its excellent
automatic ARIMA algorithm which was used in the M3-forecasting
competition \citep{M3comp00}. Another proprietary algorithm is
implemented in \pkg{Autobox} \citep{Reilly00}. \citet{OL96} provide an
early review of some of the commercial software that implement
automatic ARIMA forecasting.

\subsection{Choosing the model order using unit root tests and the AIC}

A non-seasonal ARIMA($p,d,q$) process is given by
\begin{eqnarray*}
\phi(B)(1-B^d)y_{t} &=& c + \theta(B)\varepsilon_t%\label{eq:arima1}
\end{eqnarray*}
...
\begin{eqnarray*}
\Phi(B^m)\phi(B)(1-B^{m})^D(1-B)^dy_{t} &=& c + \Theta(B^m)\theta(B)\varepsilon_t%\label{eq:arima1}
\end{eqnarray*}
...
which is outlined in \citet{DKbook01} and implemented in the
\code{arima()} function \citep{Ripley:2002} in \R. In this approach, the initial values
...
harms forecasts \citep{SY94} and widens prediction intervals.
\citep[Although, see][for a contrary view.]{Hendry97}
...
variations on the Dickey-Fuller test \citep{DF81} all assume there
is a unit root at lag 1, and the HEGY test of \citet{HEGY90} is
...
is selected based on successive KPSS unit-root tests \citep{KPSS92}.
...
extended Canova-Hansen test \citep{CH95}. Canova and Hansen only
...
\subsection{A step-wise procedure for traversing the model
space}
...
\subsection{Comparisons with exponential smoothing}
...
\citet{HAA08}. However, the non-linear exponential smoothing models
...
class is advantageous. However, the results in \citet{HKSG02} show
...
these results, \citet{Hyndman01} speculates that the larger model
...
\section[The forecast package]{The \pkg{forecast} package}\label{sec:package}
...
<<ets-forecast ,fig.height=7,fig.width=9,echo=FALSE>>=
par(mfrow = c(2,2))
plot(forecast(ets(bonds)), main="(a) US 10-year bonds yield", xlab="Year", ylab="Percentage per annum")
plot(forecast(ets(usnetelec)), main="(b) US net electricity generation", xlab="Year", ylab="Billion kwh")
plot(forecast(ets(ukcars)), main="(c) UK passenger motor vehicle production", xlab="Year", ylab="Thousands of cars")
plot(forecast(ets(visitors)), main="(d) Overseas visitors to Australia", xlab="Year", ylab="Thousands of people")
@
...
\subsection{Implementation of the automatic exponential smoothing
algorithm}

The innovations state space modelling framework described in
Section~\ref{sec:expsmooth} is implemented via the \code{ets()}
function in the \pkg{forecast} package. The models chosen via the
algorithm for the four data sets were:
\begin{compactitem}
\item ETS(A,A\damped,N) for monthly US 10-year bonds yield\\
($\alpha=0.9999$, $\beta=0.1113$, $\phi=0.8106$, $\ell_0 =  5.3338$,
$b_0=0.6097$);

\item ETS(M,A,N) for annual US net electricity generation\\
($\alpha=0.9999$, $\beta=0.1992$, $\ell_0 =  250.4317$,
$b_0=42.0242$);

\item ETS(A,N,A) for quarterly UK motor vehicle production\\
($\alpha = 0.6267$, $\gamma=0.0001$, $\ell_0 = 313.0916$, $s_{-3}=24.1778$,
$s_{-2}=21.5553$, $s_{-1}=-44.606$, $s_0=-1.1271$);

\item ETS(M,A,M) for monthly Australian overseas visitors\\
($\alpha=0.6567$, $\beta=0.0012$, $\gamma=0.1712$, $\ell_0 = 87.2703$,
$b_0=2.6226$, $s_{-11} = 0.8595$, $s_{-10} = 0.8619$, $s_{-9}= 0.9993$,
$s_{-8}= 0.9317$, $s_{-7}= 0.8324$, $s_{-6}= 1.003$, $s_{-5}= 1.107$,
$s_{-4}= 1.366$, $s_{-3}= 0.9699$, $s_{-2}= 1.0656$, $s_{-1}=1.063$,
$s_0= 0.9407$).
\end{compactitem}
...
<<ets-usnetelec,eval=TRUE,echo=TRUE>>=
...
<<ets-usnetelec-print,eval=TRUE,echo=TRUE>>=
etsfit
@

Some goodness-of-fit measures \citep[defined in][]{HK06} are
obtained using \code{accuracy()}.
...
<<ets-usnetelec-fcast,eval=TRUE,echo=TRUE, fig.show="hide">>=
...
\subsection[The HoltWinters() function]{The \code{HoltWinters()} function}

There is another implementation of exponential smoothing in \R\ via
the \code{HoltWinters()} function \citep{Meyer:2002} in the \pkg{stats} package. It
...
forecasts using \code{HoltWinters()}. (See \citet{shortseasonal} for
...
by \code{predict()} appears to be based on \citet{CY91} which is an
...
\subsection{Implementation of the automatic ARIMA algorithm}


\begin{figure}[!b]
\centering

<<arima-forecast-plots ,fig.height=7,fig.width=9,echo=FALSE>>=
par(mfrow = c(2,2))
plot(forecast(auto.arima(bonds)), main="(a) US 10-year bonds yield", xlab="Year", ylab="Percentage per annum")
plot(forecast(auto.arima(usnetelec)), main="(b) US net electricity generation", xlab="Year", ylab="Billion kwh")
plot(forecast(auto.arima(ukcars)), main="(c) UK passenger motor vehicle production", xlab="Year", ylab="Thousands of cars")
plot(forecast(auto.arima(visitors)), main="(d) Overseas visitors to Australia", xlab="Year", ylab="Thousands of people")
@
...
\begin{compactitem}
\item ARIMA(2,1,2)(1,0,1)$_{12}$ with drift for monthly US 10-year bonds yield\\
($\theta_1=-1.3090$, $\theta_2=-0.701$, $\phi_1=1.5853$, $\phi_2=0.9476$, $\Theta_1=-0.2690$, $\Phi_1=0.1206$, $c=-0.0130$);

\item ARIMA(2,1,2) with drift for annual US net electricity generation\\
($\phi_1=-1.3032$, $\phi_2=-0.4332$, $\theta_1=1.5284$,
$\theta_2=0.8340$, $c=66.1585$);

\item ARIMA(2,1,0)(2,0,0)$_4$ model for quarterly UK motor vehicle production\\
($\phi_1=-0.3475$, $\phi_2 = -0.2100$, $\Phi_1=0.2559$, $\Phi_2=0.5913$);

\item ARIMA(1,0,1)(0,1,2)$_{12}$ model with drift
for monthly Australian overseas visitors\\
($\phi_1=0.8968$, $\theta_1=-0.3187$, $\Theta_1=-0.7110$,
$\Theta_2=0.1461$, $c=1.4820$).
\end{compactitem}
...
below.
<<arima-fcast-summary,eval=TRUE,echo=TRUE>>=
summary(fcast)
@
The in-sample error measures for the two models are very similar.
...
\subsection[The forecast() function]{The \code{forecast()} function}
...
requested. Fan charts \citep{Wallis99} are possible using the
combination \verb|plot(forecast(model.object, fan = TRUE))|.


\subsection{Other functions}\label{sec:other}
...
\begin{description}
\item \code{croston()} implements \citeauthor{Croston72}'s
    \citeyearpar{Croston72} method for intermittent demand
    forecasting. In this method, the time series is decomposed
    into two separate sequences: the non-zero values and the
    time intervals between non-zero values. These are then
    independently forecast using simple exponential smoothing
    and the forecasts of the original series are obtained as
    ratios of the two sets of forecasts. No prediction intervals
    are provided because there is no underlying stochastic model
    \citep{SH05}.

\item \code{theta()} provides forecasts from the Theta method
\citep{AN00}. \citet{HB03} showed that these were equivalent to a
special case of simple exponential smoothing with drift.

\item \code{splinef()} gives cubic-spline forecasts, based on
fitting a cubic spline to the historical data and extrapolating it
linearly. The details of this method, and the associated prediction
intervals, are discussed in \citet{HKPB05}.


\item \code{meanf()} returns forecasts based on the historical mean.

\item \code{rwf()} gives ``na\"{\i}ve'' forecasts equal to the most
recent observation assuming a random walk model. This function
also allows forecasting using a random walk with drift.
...
\begin{description}
\item \code{tsdisplay()} provides a time plot along with an ACF and
PACF.

\item \code{seasonplot()} produces a seasonal plot as described in
\citet{MWH3}.

\end{description}

\bibliography{JSS-paper}
